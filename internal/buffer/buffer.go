...
// It has no dependency on any rendering or windowing library so it can be unit testeed
// headlessly and reused by any future front-end (ebiten, raylib-go, go-sdl2, go-gl, ...).
package buffer
...
// Buffer holds the text content as a slice of line, each line a slice of runes.
// A line-oriented representation keeps the implementation simple while still
// giving 0(line length) edits, which is sufficient until profiling shows
// a rope/piece-table is required(see CLAUDE.md).
type Buffer struct {
	lines [][]rune
}

// New creates a Buffer from the given text.
// Lines are split on "\n";
...
// An empty buffer still has one (empty) line, matching common editor conventions.
...
// Clamp constrains pos so it always refers to a valid location in the buffer,
// clamping the line into range first and then the column.
...
// InsertNewLine splits the line at pos into two lines and
// returns the caret position at the start of the new (second) line.
...
// DeleteBackward deletes the rune immediately before pos (Backspace behavior).
// If pos is at the satrt of a line, it joins the line with the previous one instead.
// It returns the resulting caret position and
...
// DeleteForward deletes the rune at pos (Delete key behavior).
// If pos is at the end of a line, it joins the next line into the current one instead.
// It returns whether anything was deleted (false at the very end of the buffer).
