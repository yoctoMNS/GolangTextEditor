...
// App implements the ebiten.Game interface (Update / Draw / Layout are the method names that interface requires),
// driving an *editor.Editor from keyboard input and rendering its buffer as plain monospaced text.
// The type is named App, not Game, because this is a text editor front-end, not a game.
type App struct {
	Ed         *editor.Editor
	blinkTick  int
...
// New create an App around ed.
func New(ed *editor.Editor) *App {
	return &App{
		Ed:         ed,
		keyHeldFor: make(map[ebiten.Key]int),
	}
}

// Update advances the editor state by on tick in response to input.
...
// handleRepeatable calls action on the tick a key is first pressed,
// and again on a fixed interval while it is held, so navigation and deletion
