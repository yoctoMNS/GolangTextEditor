// Package editor implements editing commands and file I/O on top of a
// buffer.Buffer. It knows nothing about rendering or input devices: any
// front-end (ebiten, raylib-go, go-sdl2, go-gl, a terminal UI, tests, ...)
...
// Save writes the buffer to Path. It returns an error if Path is empty;
// use SaveAs for the first save of a new file.
...
// MoveLeft moves the caret one rune to the left, wrapping to the end of
...
// MoveRight moves the caret one rune to the right, wrapping to the start
...
// MoveUp moves the caret one line up, clamping the column to the target
// line's length.
func (e *Editor) MoveUp() {
	e.Cursor = e.Buf.Clamp(buffer.Position{Line: e.Cursor.Line - 1, Col: e.Cursor.Col})
}

// MoveDown moves the caret one line down, clamping the column to the
